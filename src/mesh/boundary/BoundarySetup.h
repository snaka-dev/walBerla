                                    ...
                                    Location boundaryLocation)
{
   for (auto& block : *structuredBlockStorage_)
   {
      ...
      for (cell_idx_t z = -cell_idx_c(numGhostLayers_); z < cell_idx_c(voxelizationField->zSize() + numGhostLayers_);
           ++z)
      {
         for (cell_idx_t y = -cell_idx_c(numGhostLayers_); y < cell_idx_c(voxelizationField->ySize() + numGhostLayers_);
              ++y)
         {
            for (cell_idx_t x = -cell_idx_c(numGhostLayers_);
                 x < cell_idx_c(voxelizationField->xSize() + numGhostLayers_); ++x)
            {
